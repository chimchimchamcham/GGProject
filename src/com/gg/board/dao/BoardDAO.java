...
public class BoardDAO {
	...
	public ArrayList<GGDto> reqlist(String userid, int reqindex) throws SQLException {
		...
		if (reqindex == 0) {// 전체
			...
			ps = conn.prepareStatement(sql);
			ps.setString(1, userid);//0은 숫자라서 불가능
		}
...
///////////////////////////////////////////////////////
	...
	public ArrayList<GGDto> auction_main_list(String auctionmainlistwhatadd, int auctionmainlisthowaline)
		...
		if (auctionmainlisthowaline == 0) {// 신규등록 순
			sql = "SELECT DISTINCT P.P_NO, P.P_ID, P.P_TITLE, a.au_endTm, H.HA_BIDUSR,a.au_count ,HM.TOPPR,I.I_NEWNAME,A.Au_startPr,A.Au_instantPr,P.P_TM,c.c_name,p.P_blindYN FROM sale s, POST P, AUCTION A,IMG I,HIS_AUCTION H,codes c,(SELECT P_NO, MAX(HA_BIDPR) TOPPR FROM HIS_AUCTION GROUP BY P_NO) HM WHERE P.P_NO = A.P_NO AND A.P_NO = HM.P_NO AND HM.P_NO = H.P_NO AND a.p_no = i.p_no and p.p_no = s.p_no and a.p_no = s.p_no and s.p_no = h.p_no AND H.HA_BIDPR = HM.TOPPR and p.p_code ='P001'  and c.c_code = s.s_code and c.c_name= ? and a.au_code = 'Au001' and p.P_blindYN = 'N' order by p.p_tm desc";
		} else if (auctionmainlisthowaline == 1) {// 마감 임박순
			sql = "SELECT DISTINCT P.P_NO, P.P_ID, P.P_TITLE, a.au_endTm, H.HA_BIDUSR,a.au_count ,HM.TOPPR,I.I_NEWNAME,A.Au_startPr,A.Au_instantPr,P.P_TM,c.c_name,p.P_blindYN FROM sale s, POST P, AUCTION A,IMG I,HIS_AUCTION H,codes c,(SELECT P_NO, MAX(HA_BIDPR) TOPPR FROM HIS_AUCTION GROUP BY P_NO) HM WHERE P.P_NO = A.P_NO AND A.P_NO = HM.P_NO AND HM.P_NO = H.P_NO AND a.p_no = i.p_no and p.p_no = s.p_no and a.p_no = s.p_no and s.p_no = h.p_no AND H.HA_BIDPR = HM.TOPPR and p.p_code ='P001'  and c.c_code = s.s_code and c.c_name= ? and a.au_code = 'Au001' and p.P_blindYN = 'N' order by a.au_endTm desc";
		}
